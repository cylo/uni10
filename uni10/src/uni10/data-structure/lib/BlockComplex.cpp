...
namespace uni10{
  ...
  void Block::exportElem(cflag tp, Complex *out_array, int elem_num){
    /// PYTHON ONLY!!!
    try{
      throwTypeError(tp);
      if(out_array == NULL){
      ...
      if(typeID() == 1){
        std::ostringstream err;
        err<<"This Matrix is REAL. Please use Block::exportElem(uni10::rflag, double *out_array, int elem_num) instead";
        throw std::runtime_error(exception_msg(err.str()));
      }
    }
    catch(const std::exception& e){
      propogate_exception(e, "In function Block::exportElem(uni10::cflag, Complex *out_array, int elem_num):");
    }
    elem_num = std::min(elem_num, (int)elemNum());
    memcpy(out_array, cm_elem, sizeof(Complex) * elem_num);
  }

  void Block::exportElemC(Complex *out_array, int elem_num){
    /// PYTHON ONLY!!!
    if (elem_num < 0)
      elem_num = elemNum();
