...
void UniTensor::exportElem(rflag tp, double *out_array, int elem_num){
  /// PYTHON ONLY!!!
  try{
    throwTypeError(tp);
    if(out_array == NULL){
    ...
    if(typeID() == 2){
      std::ostringstream err;
      err<<"This Tensor is COMPLEX. Please use UniTensor::exportElem(uni10::cflag, Complex *out_array, int elem_num) instead";
      throw std::runtime_error(exception_msg(err.str()));
    }
  }
  catch(const std::exception& e){
    propogate_exception(e, "In function UniTensor::exportElem(uni10::rflag, double *out_array, int elem_num):");
  }
  elem_num = std::min(elem_num, (int)m_elemNum);
  memcpy(out_array, elem, sizeof(double) * elem_num);
}

void UniTensor::exportElemR(double *out_array, int elem_num){
  /// PYTHON ONLY!!!
  if (elem_num < 0)
    elem_num = m_elemNum;
...
UniTensor& UniTensor::permuteFm(rflag tp, int rowBondNum){
  try{
    throwTypeError(tp);
    std::vector<int> ori_labels = labels;
    this->permuteFm(RTYPE, ori_labels, rowBondNum);
  }
  catch(const std::exception& e){
    propogate_exception(e, "In function UniTensor::permuteFm(uni10::rflag, int):");
  }
  return *this;
}

UniTensor& UniTensor::permuteFm(rflag tp, int* newLabels, int rowBondNum){
  try{
    throwTypeError(tp);
    std::vector<int> _labels(newLabels, newLabels + bonds.size());
    this->permuteFm(RTYPE, _labels, rowBondNum);
  }
  catch(const std::exception& e){
    propogate_exception(e, "In function UniTensor::permuteFm(uni10::rflag, int*, int):");
  }
  return *this;
}

UniTensor& UniTensor::permuteFm(rflag tp, const std::vector<int>& newLabels, int rowBondNum){
  try{
    throwTypeError(tp);
    if((status & HAVEBOND) == 0){
      std::ostringstream err;
    ...
    propogate_exception(e, "In function UniTensor::permuteFm(uni10::rflag, std::vector<int>&, int):");
  }
  return *this;
}
...
void UniTensor::initUniT(rflag tp){ //GPU
  r_flag = RTYPE;
  c_flag = CNULL;
  if(bonds.size()){
